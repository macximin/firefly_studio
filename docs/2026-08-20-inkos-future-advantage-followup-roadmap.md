...
- 현재 기준: P0·P1 저장 완료, P2-A·P2-B·P3·P4·P5·P6·P7 구현 및 검증 완료
...
### 2026-08-20 P6 검증 영수증

- 기존 기획서 화면이 `book_rules.md` 정본에서 미래 선점 계약의 기준 시점·핵심 재미·허용 분야·정보 경계·금지 지름길·기억/고증 정책을 읽기 전용 표시
- 기존 Arc 지도 상세 패널의 A 레일과 B 레일에 실행·증거·보상 및 저항·후폭풍·기억 열화를 함께 표시
- 기존 작품 상세 화면에 마지막 회차 감리 결과를 창작 통과와 고증 상태로 분리 표시
- 미래 선점 실행 후보가 있으면 move ID와 기억 신뢰도를 같은 감리 결과에 표시
- Studio 검수 버튼을 Core 정식 `auditDraft` 경로로 통합해 수동 편집 후 재감리가 비정본 실행 후보를 실제로 갱신
- 합성 감리 단계가 LLM Auditor의 본문 근거 실행 후보를 버리지 않도록 보존하고 전용 회귀 테스트 추가
- 새 페이지·대시보드·작품 데이터 파일 생성 없음
- 전체 테스트: Core 2015 + Studio 642 + CLI 243 = 2900 PASS
- Core·Studio·CLI 타입 검사: PASS
- Core·Studio·CLI 프로덕션 빌드: PASS
- 패키지 manifest 검증 및 변경문법 검사: PASS
- semantic pattern audit: 신규 후보 없음(기존 후보 26개만 보고)
...
### 2026-08-20 P7 검증 영수증

- 기술·금융·경영·유통·문화·인재·정책·복합 8개 분야의 허용된 조기 선점 카나리 PASS
- `실제 역사보다 빠르다`는 이유만으로 창작 탈락한 사례 0개
- 새로 생성되는 미래 선점 move는 구현 다리·저항·증거·보상·후폭풍과 함께 구체적인 `memoryRisk`를 필수 검증하고, 기존 저장 자료는 optional 필드로 하위 호환
- 대가 없는 무손실 예언은 기억 위험 누락 gate, 구현 생략은 구현 다리 누락 gate로 각각 수정 요구
- 정보 경계 위반은 기존 창작 critical 차단 경로를 유지
- snapshot 복원 뒤 최신 회차를 삭제하는 복합 경로에서도 작품 장부와 별도 고증 receipt가 같은 시점으로 복원
...
- semantic pattern audit: 신규 후보 없음(기존 후보 26개만 보고)
- 원고·작품 데이터·Tavily 키·자격 증명: 변경 없음
- 《IMF를 독식한 재벌 3세》: 자동 적용하지 않았으며 명시적인 사람 승인 대기
